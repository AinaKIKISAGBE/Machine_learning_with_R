setwd("H:/Documents/big data/test_svm_lineaire_sortie")
...
M1=matrix(c("nb_cost","erreur"),1,2)
write.table(M1, "ressource_suivi_svm_lineaire.csv", row.names=F,col.names = F, sep="\t",dec=",", na=" ",append = TRUE)


ancien_suivi <- read.csv("ressource_suivi_svm_lineaire.csv", header = T, sep="\t",dec=",", na=" ")
ancien_suivi$nb_cost<-as.character(ancien_suivi$nb_cost)

ancien_suivi$list_suivi<-ancien_suivi$nb_cost
...
system.time(svmlin <- svm(Xfinal, Y, kernel="linear", probability=TRUE, cost=nb_cost))
print(svmlin)
summary(svmlin)
svmlin$rho
...
M=matrix(c(nb_cost,erreur),1,2)

# M2<-rbind(M1,M)
t_sorti<-as.data.frame(M)
write.table(t_sorti, "ressource_suivi_svm_lineaire.csv", row.names=F,col.names = F, sep="\t",dec=",", na=" ",append = TRUE)
 #          }
 #       }
 #     }
 #   }
